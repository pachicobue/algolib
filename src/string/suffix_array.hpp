...
template <typename V> auto suffixArray(const V& vs) -> Vec<int> {
    ...
    for (int i : per(N)) {
        is_s[i] = val(i) == val(i + 1) ? is_s[i + 1] : val(i) < val(i + 1);
    }
    auto isS = [&](int i) {
        return is_s[i];
    };
    ...
    auto isLms = [&](int i) {
        return i > 0 and isL(i - 1) and isS(i);
    };
    ...
    auto inducedSort = [&](const Vec<int>& lmss) {
        ...
        std::for_each(std::rbegin(lmss), std::rend(lmss), [&](int lms) {
            pushS(lms);
        });
        // 上からLを詰める
        for (int b : rep(B + 1)) {
    ...
    if (LMS > 1) {
        ...
        for (int ii : irange(1, LMS)) {
            ...
            if (sameLmsSubstr(plms, lms)) {
                kinds[lms]--;
            }
        }
        Vec<int> sub_vs(LMS);
        for (int i : rep(LMS)) {
            sub_vs[i] = kinds[lmss[i]];
        }
        // LMSをソートする
        const auto lmss_ords = suffixArray(sub_vs);
        for (int i : rep(LMS)) {
            nlmss[i] = lmss[lmss_ords[i]];
        }
        // 正しいLMS順でIS
        inducedSort(nlmss);
    }
