...
data:
  ...
  bundledCode: "#include <bits/stdc++.h>\n#pragma region Macros\n#pragma endregion\n\
    ...
    #pragma endregion\ntemplate<typename T>\nVec<T> setHadamard(const Vec<T>& xs,\
  ...
  code: "#pragma once\n#include \"../misc/common.hpp\"\ntemplate<typename T>\nVec<T>\
    \ setHadamard(const Vec<T>& xs, bool rev = false)\n{\n    const int N = ceil2(xs.size());\n\
    \    Vec<T> ys(N);\n    for (int i : rep(xs.size())) {\n        ys[i] = xs[i];\n\
    \    }\n    for (int i = 1; i < N; i <<= 1) {\n        for (int j : rep(N)) {\n\
    \            if ((j & i) == 0) {\n                const T x = ys[j], y = ys[j\
    \ | i];\n                ys[j] = x + y, ys[j | i] = x - y;\n            }\n  \
    \      }\n    }\n    if (rev) {\n        const T iN = T{1} / N;\n        for (auto&\
    \ y : ys) {\n            y *= iN;\n        }\n    }\n    return ys;\n}\n"
  dependsOn:
...
documentation_of: src/math/set_hadamard.hpp
layout: document
redirect_from:
- /library/src/math/set_hadamard.hpp
- /library/src/math/set_hadamard.hpp.html
