...
data:
  ...
  bundledCode: "#include <bits/stdc++.h>\nusing i32 = int;\nusing u32 = unsigned int;\n\
    ...
    \ N) : m_N{N}, m_sqrt{(u32)std::sqrt(N)}, m_ns{0}\n    {\n        assert(N >=\
    \ 2);\n        for (u64 i = N; i > 0; i = N / (N / i + 1)) {\n            m_ns.push_back(i);\n\
    \        }\n        for (const auto n : m_ns) {\n            const T x = n;\n\
    \            m_hss[0].push_back(x - 1);\n            m_hss[1].push_back(x * (x\
    \ + 1) / 2 - 1);\n            m_hss[2].push_back(x * (x + 1) * (x * 2 + 1) / 6\
    \ - 1);\n        }\n        for (u32 x = 2; x <= m_sqrt; x++) {\n            if\
    \ (m_hss[0][m_ns.size() - x] == m_hss[0][m_ns.size() - x + 1]) {\n           \
    \     continue;\n            }\n            m_ps.push_back(x);\n            const\
    ...
    \ -= (m_hss[0][j] - m_hss[0][k]);\n                m_hss[1][i] -= (m_hss[1][j]\
    \ - m_hss[1][k]) * x;\n                m_hss[2][i] -= (m_hss[2][j] - m_hss[2][k])\
    \ * x * x;\n            }\n        }\n    }\n    T get(u32 d, u64 n) const\n \
    \   {\n        const u32 i = id(n);\n        assert(m_ns[i] == n);\n        return\
    \ m_hss[d][i];\n    }\nprotected:\n    u32 id(u64 x) const\n    {\n        return\
    \ x <= m_sqrt ? m_ns.size() - x : m_N / x;\n    }\n    u64 m_N;\n    u32 m_sqrt;\n\
  ...
  code: "#pragma once\n#include \"../misc/common.hpp\"\n#include \"prime_powsum.hpp\"\
    ...
    \    using PrimePowSum<T>::m_hss;\n\npublic:\n    static constexpr u32 MAXD =\
    \ PrimePowSum<T>::MAXD;\n    MultiplicativeSum(u64 N, F f, const Arr<T, MAXD +\
    \ 1>& coeffs)\n        : PrimePowSum<T>{N}, m_buf(m_ns.size(), 0), m_f{f}\n  \
