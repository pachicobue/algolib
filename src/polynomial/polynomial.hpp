...
public:
     ...
     */
    auto low(i64 n) const -> Polynomial {
        const int sz = (int)std::ranges::min(n, (i64)size());
     ...
     */
    auto inv(int n) const -> Polynomial {
        assert(n >= 1);
        assert((*this)[0].val() != 0);
     ...
     */
    auto exp(int n) const -> Polynomial {
        assert(n >= 1);
        assert((*this)[0] == 0);
        Polynomial G{1};
        for (int m = 1; m < n; m *= 2) {
     ...
     */
    auto taylorShift(const mint& c) const -> Polynomial {
...
#ifdef HOGEPACHI
    friend Ostream&
        operator<<(Ostream& os, const Polynomial& f) {
        return os << f.asVec();
    }
#endif
private:
