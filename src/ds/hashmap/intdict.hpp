...
private:
    ...
    {
        IntDict newDict(new_cap);
        for (int i : rep(m_cap)) {
            if (m_shifts[i] != -1) { newDict[m_keys[i]] = m_vals[i]; }
        }
        std::swap(newDict, *this);
    }
    void expand()
    {
    ...
    void alloc(K k)
    {
        ...
        for (int ind = hash(k), s = 0;; (ind += 1) &= (m_cap - 1)) {
            if (m_shifts[ind] == -1) {
                m_keys[ind] = k, m_vals[ind] = v, m_shifts[ind] = s;
                return;
            }
            if (m_shifts[ind] < s) {
                std::swap(m_keys[ind], k), std::swap(m_vals[ind], v), std::swap(m_shifts[ind], s);
            }
            s++;
        }
    }
