...
template<typename mint> class FormalPowerSeries : public Vec<mint>
{
...
public:
    ...
    template<typename I> void shrink(I n)
    {
        if (n >= (I)size()) { return; }
        Vec<mint>::resize(n);
        optimize();
    }
    template<typename I> FormalPowerSeries low(I n) const
    {
    ...
    template<typename I> mint at(const I n) const { return (n < size() ? (*this)[n] : mint{0}); }
    ...
    FormalPowerSeries& operator>>=(const int s) { return *this = (*this >> s); }
    FormalPowerSeries operator+(const FormalPowerSeries& f) const { return FormalPowerSeries(*this) += f; }
    FormalPowerSeries operator-(const FormalPowerSeries& f) const { return FormalPowerSeries(*this) -= f; }
    FormalPowerSeries operator*(const FormalPowerSeries& f) const { return mult(f, size() + f.size() - 1); }
    ...
    friend Ostream& operator<<(Ostream& os, const FormalPowerSeries& f) { return os << static_cast<Vec<mint>>(f); }
    ...
    template<typename I> FormalPowerSeries pow(I n) const { return pow(n, deg() * n + 1); }
    template<typename I> FormalPowerSeries pow(I n, int size) const
    {
        assert(size > 0);
        ...
        return ((f.log(size) * FormalPowerSeries{n}).exp(size) * FormalPowerSeries{c.pow(n)}) >> (k * n);
    }
private:
