 ...
 */
template <typename mint> auto modNthRoot(mint A, i64 K) -> Opt<mint> {
    constexpr auto Null = std::nullopt;
    const i64 P = mint::mod();
    if (K == 0) {
        return A == 1 ? Opt<mint>{1_i64} : Opt<mint>{Null};
    }
    if (A == 0 or A == 1) {
        return A;
    }
    ...
    for (const auto& [p, a, b] : factors) { // (p^a)*log(X)-log(A)=0 (mod P-1) を解く (Aを更新しながら順に解く)
        ...
        for (mint Z = 2;; Z += 1) {
            Eraser = Z.pow(Q);
            if (ordLog_p(Eraser) == 0) {
                break;
            }
        }
        Vec<mint> pErasers(b, Eraser); // pEraser[i] = Eraser^(p^i)
        for (int i : rep(b - 1)) {
            pErasers[i + 1] = pErasers[i].pow(p);
        }
        ...
        for (mint E = 1, ER = pErasers[b - 1].pow(W); i64 x : rep(M)) {
            Memo[E.val()] = W * x, E *= ER;
        }
        auto logErase = [&](mint X) { // X=Erase^(k*p^{b-1}) となる k を返す
            for (i64 i : rep(W + 1)) {
                if (Memo.contains(X.val())) {
                    return Memo[X.val()] + i;
                }
                X *= ipEraser;
            }
