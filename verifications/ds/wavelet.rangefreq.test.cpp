...
int main()
{
    ...
    Vec<i64> ls(N), rs(N);
    for (int i : rep(N)) { std::tie(ls[i], rs[i]) = in.tup<i64, i64>(); }

    auto is = iotaVec(N);
    sortAll(is, [&](int i, int j) { return as[i] < as[j]; });
    constexpr i64 L = TEN<i64>(9);
    Vec<i64> bs = as;
    for (int i : rep(N)) { bs[i] = as[is[i]] - ls[is[i]] + L; }
    auto wm = WaveletMatrix(bs, 31);

    i64 ans = 0;
    for (int i : rep(N - 1)) {
        const int jsup = binSearch(i, N, [&](int j) { return as[is[i]] + rs[is[i]] < as[is[j]]; });
