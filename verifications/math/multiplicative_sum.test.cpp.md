...
data:
  ...
  bundledCode: "#include <bits/stdc++.h>\n#include <iostream>\n#pragma region Macros\n\
    ...
    \    {\n        assert(N >= 2);\n        for (u64 i = N; i > 0; i = N / (N / i\
    \ + 1)) {\n            m_ns.push_back(i);\n        }\n        for (const auto\
    \ n : m_ns) {\n            const T x = n;\n            m_hss[0].push_back(x -\
    \ 1);\n            m_hss[1].push_back(x * (x + 1) / 2 - 1);\n            m_hss[2].push_back(x\
    \ * (x + 1) * (x * 2 + 1) / 6 - 1);\n        }\n        for (u32 x = 2; x <= m_sqrt;\
    \ x++) {\n            if (m_hss[0][m_ns.size() - x] == m_hss[0][m_ns.size() -\
    \ x + 1]) {\n                continue;\n            }\n            m_ps.push_back(x);\n\
    ...
    \    T get(u64 n) const\n    {\n        T ans = m_buf[id(n)] + 1;\n        for\
    \ (u32 i : rep(m_ps.size())) {\n            ans += dfs(i, 1, m_ps[i], n / m_ps[i],\
    \ 1);\n        }\n        return ans;\n    }\nprivate:\n    T dfs(u32 i, u32 c,\
    \ u64 v, u64 lim, T fv) const\n    {\n        T ans = fv * m_f(m_ps[i] * v, m_ps[i],\
    \ c + 1);\n        if (lim >= m_ps[i] * m_ps[i]) {\n            ans += dfs(i,\
    \ c + 1, m_ps[i] * v, lim / m_ps[i], fv);\n        }\n        fv *= m_f(v, m_ps[i],\
    \ c);\n        ans += fv * (m_buf[id(lim)] - m_buf[id(m_ps[i])]);\n        for\
    \ (u32 j = i + 1; j < m_ps.size() and m_ps[j] * m_ps[j] <= lim; j++) {\n     \
    ...
    \ u64 v, const u64 p, u32) { return v / p * (p - 1); };\n    const auto ms = MultiplicativeSum<mint,\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/sum_of_totient_function\"\
    ...
    \    if (N <= 2) { return out.ln(N); }\n    auto f = [&](const u64 v, const u64\
    \ p, u32) { return v / p * (p - 1); };\n    const auto ms = MultiplicativeSum<mint,\
    \ decltype(f)>(\n        N, f, {mint(-1), mint(1), mint(0)});\n    out.ln(ms.get(N).val());\n\
    \    return 0;\n}\n"
  dependsOn:
  - src/math/modint.hpp
...
layout: document
redirect_from:
- /verify/verifications/math/multiplicative_sum.test.cpp
- /verify/verifications/math/multiplicative_sum.test.cpp.html
