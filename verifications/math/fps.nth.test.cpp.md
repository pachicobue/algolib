...
data:
  ...
  bundledCode: "#include <iostream>\n#include <bits/stdc++.h>\n#pragma region Macros\n\
    ...
    #pragma endregion\ntemplate<typename T>\nVec<T> berlekampMassey(const Vec<T>&\
    ...
    \      B = temp, b = d;\n        } else {\n            for (int i : rep(m)) {\n\
    \                C[l - 1 - i] += c * B[m - 1 - i];\n            }\n        }\n\
    \    }\n    reverseAll(C);\n    return C;\n}\ntemplate<u32 mod_, u32 root_, u32\
    ...
    \     }\n    }\n    mat css(K, vec(K, 0));\n    for (int i = 0; i < K - 1; i++)\
    \ {\n        css[i][i + 1] = 1;\n    }\n    for (int j = 0; j < K; j++) {\n  \
    \      css[K - 1][j] = 1;\n    }\n    auto mul = [&](const mat& m1, const mat&\
    \ m2) {\n        mat ans(K, vec(K, 0));\n        for (int i = 0; i < K; i++) {\n\
    \            for (int j = 0; j < K; j++) {\n                for (int k = 0; k\
    \ < K; k++) {\n                    ans[i][j] += m1[i][k] * m2[k][j];\n       \
    \         }\n            }\n        }\n        return ans;\n    };\n    auto pow\
    \ = [&](auto self, const mat& m, const i64 n) -> mat {\n        if (n == 1) {\
    \ return m; }\n        if (n % 2 == 0) {\n            return self(self, mul(m,\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/aplusb\"\n#include \"../../src/math/nth_term.hpp\"\
    ...
    \    for (int i = 0; i < K - 1; i++) {\n        css[i][i + 1] = 1;\n    }\n  \
    \  for (int j = 0; j < K; j++) {\n        css[K - 1][j] = 1;\n    }\n    auto\
    \ mul = [&](const mat& m1, const mat& m2) {\n        mat ans(K, vec(K, 0));\n\
    \        for (int i = 0; i < K; i++) {\n            for (int j = 0; j < K; j++)\
    \ {\n                for (int k = 0; k < K; k++) {\n                    ans[i][j]\
    \ += m1[i][k] * m2[k][j];\n                }\n            }\n        }\n     \
    \   return ans;\n    };\n    auto pow = [&](auto self, const mat& m, const i64\
    \ n) -> mat {\n        if (n == 1) { return m; }\n        if (n % 2 == 0) {\n\
    \            return self(self, mul(m, m), n / 2);\n        } else {\n        \
    \    return mul(m, self(self, m, n - 1));\n        }\n    };\n    const auto Mat\
    \ = pow(pow, css, N);\n    mint actual = 0;\n    for (int j = 0; j < K; j++) {\n\
    \        actual += Mat[0][j] * as[j];\n    }\n    const mint ans = nthTerm(as,\
    \ N);\n    assert(actual == ans);\n    out.ln(A + B);\n    return 0;\n}\n"
  dependsOn:
...
layout: document
redirect_from:
- /verify/verifications/math/fps.nth.test.cpp
- /verify/verifications/math/fps.nth.test.cpp.html
